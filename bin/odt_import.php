...
$OPHIR_CONF = $odt_import_conf;

function odt_import_is_image ($file) {
...
function odt_import_copy_file($from, $to) {
...
function odt_import_error($error){
...
function odt2html($odt_file, $xml_string=NULL) {
  global $odt_import_conf;
  
  $xml = new XMLReader();

  if ($xml_string===NULL){
    if (@$xml->open('zip://'.$odt_file.'#content.xml') === FALSE) {
      odt_import_error("Unable to read file contents.");
      return false;
    }
  }else{
    if(@$xml->xml($xml_string)===FALSE) {
      odt_import_error("Invalid file contents.");
      return false;
    }
  }
  ...
  if ($odt_import_conf["features"]["list"]===0) $translation_table["text:list"] = FALSE;
  elseif ($odt_import_conf["features"]["list"]===2) {
    $translation_table["text:list"] = "ul";
    $translation_table["text:list-item"] = "li";
  }
  if ($odt_import_conf["features"]["table"]===0) $translation_table["table:table"] = FALSE;
  elseif ($odt_import_conf["features"]["table"]===2) {
  ...
  if ($odt_import_conf["features"]["table of contents"]===0) $translation_table['text:table-of-content'] = FALSE;
  elseif ($odt_import_conf["features"]["table of contents"]===2) {
    ...
    elseif (in_array($xml->nodeType,
      ...
      switch ($xml->name) {
        case "#text"://Text
          ...
          break;
        case "text:h"://Title
          if ($odt_import_conf["features"]["header"]===0) {
            $xml->next();
            break;
          }
          elseif ($odt_import_conf["features"]["header"]===1) break;
        ...
        case "text:a":
          if ($odt_import_conf["features"]["link"]===0) {
            $xml->next();
            break;
          }
          elseif ($odt_import_conf["features"]["link"]===1) break;
        ...
        case "draw:image":
          if ($odt_import_conf["features"]["image"]===0) {
            $xml->next();
            break;
          }
          elseif ($odt_import_conf["features"]["image"]===1) break;

          $image_file = 'zip://' . $odt_file . '#' . $xml->getAttribute("xlink:href");
          if (isset($odt_import_conf["images_folder"]) &&
              is_dir($odt_import_conf["images_folder"]) ) {
            if (odt_import_is_image($image_file)) {
              $image_to_save = $odt_import_conf["images_folder"] . '/' . basename($image_file);
              if ( !($src = odt_import_copy_file ($image_file, $image_to_save))) {
                odt_import_error("Unable to move image file");
                break;
              } 
            } else {
              odt_import_error("Found invalid image file.");
              break;
            } 
          }
          else {
            //odt_import_error('Unable to save the image. Creating a data URL. Image saved directly in the body.F');
        ...
        case "style:style":
          ...
          break;
        case "text:note":
          if ($odt_import_conf["features"]["note"]===0) {
            $xml->next();
            break;
          }
          elseif ($odt_import_conf["features"]["note"]===1) break;
        ...
        case "office:annotation":
          if ($odt_import_conf["features"]["annotation"]===0) {
    $xml->next();
    break;
    }
          elseif ($odt_import_conf["features"]["annotation"]===1) break;
